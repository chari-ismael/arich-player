...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  void _openSettings() {
    if (!_isTV && MediaQuery.of(context).orientation == Orientation.portrait) {
            ...
            child: Container(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Text(context.read<LanguageProvider>().l10n.t('player_playing'),
                    style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 16),
                  ..._mobileSettingsContent(sizes),
                ],
              ),
            ),
          ),
  ...
  List<Widget> _mobileSettingsContent(TVSizes sizes) {
    final l = context.read<LanguageProvider>().l10n;
    return [
      if (!_isLive) ...[
        _label(l.t('player_speed'), sizes),
      ...
      if (_isLive && widget.liveChannels != null) ...[
        _label('Multi-écrans', sizes),
        const SizedBox(height: 8),
        _chip('Ouvrir', selected: false, onTap: () {
      ...
      if (!_isTizen) ...[
        _label('Picture-in-Picture', sizes),
        const SizedBox(height: 8),
        _chip('Activer', selected: false, onTap: () { Navigator.pop(context); _enterPip(); }, sizes: sizes),
      ],
    ];
  }
      ...
      child: GestureDetector(
        ...
        child: Stack(
          ...
          children: [
            ...
            if (_showSettings && (_isTV || MediaQuery.of(context).orientation == Orientation.landscape))
              Positioned.fill(child: GestureDetector(onTap: _closeSettings,
                  child: Container(color: Colors.transparent))),
            if (_isTV || MediaQuery.of(context).orientation == Orientation.landscape)
