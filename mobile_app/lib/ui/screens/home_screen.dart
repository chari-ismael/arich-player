...
class _HomeScreenState extends State<HomeScreen> {
        ...
        Widget sheet = Container(
          ...
          child: SafeArea(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              _PlusMenuTile(icon: Icons.favorite_rounded, color: AppTheme.red,
                  label: l.t('nav_favorites'), onTap: () {
                Navigator.pop(ctx);
                Navigator.push(context, _fadeRoute(const FavoritesScreen()));
              }),
              _PlusMenuTile(icon: Icons.calendar_view_week_rounded, color: AppTheme.secondary,
                  label: l.t('nav_tv_guide'), onTap: () {
                Navigator.pop(ctx);
                Navigator.push(context, _fadeRoute(EpgScreen()));
              }),
              _PlusMenuTile(icon: Icons.download_rounded, color: AppTheme.violet,
                  label: l.t('downloads_title'), onTap: () {
                Navigator.pop(ctx);
                Navigator.push(context, _fadeRoute(const DownloadsScreen()));
              }),
              _PlusMenuTile(icon: Icons.bar_chart_rounded, color: AppTheme.gold,
                  label: 'Statistiques', onTap: () {
                Navigator.pop(ctx);
                Navigator.push(context, _fadeRoute(const StatsScreen()));
              }),
              _PlusMenuTile(icon: Icons.settings_rounded, color: Colors.white70,
                  label: l.t('settings_title'), onTap: () {
                Navigator.pop(ctx);
                Navigator.push(context, _fadeRoute(const SettingsScreen()))
                    .then((_) { if (mounted) setState(_loadHidePrefs); });
              }),
  ...
  List<({IconData icon, String label, int index})> _bottomNavData() {
    final l = context.read<LanguageProvider>().l10n;
    return [
      (icon: Icons.home_rounded, label: l.t('nav_home'), index: 0),
      if (!_hideLive)   (icon: Icons.live_tv_rounded, label: l.t('nav_live'), index: 1),
      if (!_hideMovies) (icon: Icons.movie_rounded, label: l.t('nav_movies'), index: 2),
      if (!_hideSeries) (icon: Icons.tv_rounded, label: l.t('nav_series'), index: 3),
      (icon: Icons.more_horiz_rounded, label: 'Plus', index: 100),
    ];
  }

  bool _shouldShowRow(List<Channel> items, IptvProvider provider) =>
      items.isNotEmpty || provider.isLoading || provider.isPrefetchingContent;
  ...
  void _onTabTap(int index) {
    ...
    if (index == 100) { _openPlusMenu(); return; }
  ...
  Widget _buildPortrait(BuildContext context, IptvProvider provider) {
    ...
    if (_activeTab > 0 && _activeTab < 90) {
      ...
      return Stack(children: [
        ...
        _buildNavbar(provider, solid: true, portraitMode: true),
        Positioned(left: 0, right: 0, bottom: 0, child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _PortraitBottomNav(tabs: _bottomNavData(), activeTab: _activeTab, onTabTap: _onTabTap),
        ])),
      ]);
    }
    ...
    return Stack(children: [
      CustomScrollView(
        ...
        slivers: [
          ...
          if (!_hideLive && provider.allLive.isNotEmpty)
            SliverToBoxAdapter(child: _buildTonightSection(provider)),
          SliverToBoxAdapter(child: _buildContinueWatching(provider)),
          if (!_hideMovies && _shouldShowRow(provider.allMovies, provider))
          ...
          if (!_hideSeries && _shouldShowRow(provider.allSeries, provider))
            SliverToBoxAdapter(child: _buildRow(
      ...
      _NavbarScrollWrapper(
        ...
        heroH: _kHeroH,
        builder: (solid) => _buildNavbar(provider, solid: solid, portraitMode: true),
      ),
      Positioned(left: 0, right: 0, bottom: 0, child: Column(mainAxisSize: MainAxisSize.min, children: [
        MiniPlayerBar(onExpand: _expandMiniPlayer),
        _PortraitBottomNav(
  ...
  // [B] NAVBAR
  // ══════════════════════════════════════════════════════════════════════════
  Widget _buildNavbar(IptvProvider provider, {bool solid = false, bool portraitMode = false}) {
    final isTizen = _isTizen;

    Widget inner = Container(
      decoration: BoxDecoration(
        color: solid
            ? Colors.black.withOpacity(isTizen ? 0.97 : 0.55)
            : Colors.transparent,
        border: solid
      ...
      child: SafeArea(
        ...
        child: SizedBox(
          ...
          child: AnimatedSwitcher(
            ...
            child: _isSearching ? _buildSearchBar(provider) : _buildNavRow(provider, portraitMode: portraitMode),
          ),
        ),
      ),
    );
  ...
  Widget _buildNavRow(IptvProvider provider, {bool portraitMode = false}) {
    return Row(key: const ValueKey('nav'), children: [
      ...
      Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(padding: const EdgeInsets.only(right: 10, left: 2),
  ...
  Widget _buildRow({
    ...
    double? cardW,
    double? cardH,
    bool liveStyle = false,
  }) {
    final sizes  = context.tvSizes;
    final isTV   = sizes.isTV;
    final isLand = MediaQuery.of(context).orientation == Orientation.landscape;

    final w = cardW ?? (isLand ? 90.0  : isTV ? 136.0 : 116.0);
    final h = cardH ?? (isLand ? 134.0 : isTV ? 200.0 : 172.0);
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(
          height: h,
          child: items.isEmpty
              ? (provider.isLoading || provider.isPrefetchingContent
                  ? _skeletonRow(w, h, sizes)
              ...
              : ShaderMask(
                  ...
                  child: ListView.builder(
                    ...
                    itemExtent: w + sizes.cardSpacing,
                    cacheExtent: w * 3,
                    itemCount: items.length,
                    itemBuilder: (ctx, i) {
                      final item = items[i];
                      Widget card = _PosterCard(
                        channel: item, width: w, height: h,
                        ...
                        child: Focus(
                          ...
                          onFocusChange: (focused) {
                            if (focused && isTV) {
                              ...
                              if (ctrl.hasClients) {
                                final off = (i.toDouble() * (w + sizes.cardSpacing)) - sizes.bodyPadding;
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // [D2] CE SOIR — chaînes live en direct
  // ══════════════════════════════════════════════════════════════════════════
  Widget _buildTonightSection(IptvProvider provider) {
    final sizes = context.tvSizes;
    final lives = provider.allLive.take(12).toList();
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Row(children: [
            Text(context.read<LanguageProvider>().l10n.t('nav_sport'), style: TextStyle(
        ...
        return AnimatedSwitcher(
          ...
          child: CachedNetworkImage(
            ...
            memCacheWidth: (MediaQuery.sizeOf(context).width *
                MediaQuery.devicePixelRatioOf(context)).round().clamp(720, 1440),
      ...
      child: GestureDetector(onTap: widget.onTap,
        child: AnimatedContainer(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Text(widget.label, style: TextStyle(color: widget.active ? Colors.white : Colors.white60,
                fontSize: 13, fontWeight: widget.active ? FontWeight.w600 : FontWeight.w400)),
            if (widget.active) ...[
              const Spacer(),
...
// ═════════════════════════════════════════════════════════════════════════════
// PORTRAIT BOTTOM NAV — glassmorphism
// ═════════════════════════════════════════════════════════════════════════════
class _PortraitBottomNav extends StatelessWidget {
  final List<({IconData icon, String label, int index})> tabs;
  final int activeTab;
  final void Function(int) onTabTap;
  const _PortraitBottomNav({required this.tabs, required this.activeTab, required this.onTabTap});
  ...
  Widget build(BuildContext context) {
    ...
    Widget inner = Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF06060F).withOpacity(isTizen ? 0.97 : 0.72),
            ...
            return Expanded(child: Material(
              ...
              child: InkWell(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(t.icon, size: 20, color: active ? AppTheme.violet : Colors.white38),
                  const SizedBox(height: 3),
    ...
    if (isTizen) return inner;
    return ClipRRect(
      borderRadius: BorderRadius.circular(18),
      child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 20, sigmaY: 20), child: inner),
    );
  }
}
...
class _PlusMenuTile extends StatelessWidget {
  ...
  final String label;
  final VoidCallback onTap;
  const _PlusMenuTile({required this.icon, required this.color, required this.label, required this.onTap});
